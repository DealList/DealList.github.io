...
# 주관 (38개): 시트 헤더 (ECM Table.xlsx row 2) 와 1:1 일치 — 변경 시 함께 갱신.
# (뒤 3개 골드만삭스/씨티/케이프 는 자동등록되어 시트에 컬럼이 추가된 broker — config 도 동기화)
LEAD_ECM = [
    ...
    "골드만삭스증권서울지점", "씨티그룹글로벌마켓", "케이프",
]
# 인수 (41개): 시트 헤더 (ECM Table.xlsx row 2) 와 1:1 일치 (뒤 2개 자동등록)
UW_ECM = [
