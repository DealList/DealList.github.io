...
(function () {
  ...
  function updateKPI(list) {  // 조회 기간 주요 정보 위젯 — 완료된 딜만 집계 (표 목록과 별개)
    ...
    const L = state.tab==="ipo" ? {
      cnt: "조회 기간 내 완료 IPO 건수",
      amt: "조회 기간 내 완료 IPO 금액", amtSub: "발행 총액 합산",
      avg: "조회 기간 내 완료 IPO 평균 금액",
      max: "조회 기간 내 완료 IPO 중 최대 규모",
    } : {
      cnt: "조회 기간 내 유상증자 건수(최소 1차 발행가액 확정 기준)",
      amt: "조회 기간 내 유상증자 금액(최소 1차 발행가액 확정 기준)", amtSub: "발행 총액 합산",
      avg: "조회 기간 내 유상증자 평균 금액(최소 1차 발행가액 확정 기준)",
      max: "조회 기간 내 유상증자 중 최대 규모(최소 1차 발행가액 확정 기준)",
    };
