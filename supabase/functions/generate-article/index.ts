...
- **형식: "[발행사명], [발행규모][종류] 발행…[핵심 사실 1개]"**
  ...
  - **딜 규모·종류 뒤에 말줄임표(…, 한 글자)를 찍고 그 뒤에 객관적 핵심 사실 1개를 덧붙인다.** 단순 "OO억 공모채 발행"으로 끝내는 밋밋한 제목 금지.
    - 둘째 정보는 **가장 특징적인 사실 하나만**: 증액 규모 / 만기 구성 / 수요예측 경쟁률 / 최종·언더 금리 등. 전부 사실, 평가·감정어 금지.
    - **증액을 쓸 경우 반드시 '증액_총_억'(회차 합산 총증액) 값**으로 쓴다. 트랜치별 증액액(예: 특정 만기 1100억)을 증액 규모로 쓰면 안 된다. (예: 회차 총 600억 증액 → "…600억 증액")
  - 예: "롯데쇼핑, 2600억 공모채 발행…600억 증액"
  - 예: "NH투자증권, 3000억 공모채 발행…2·3년물 구성"
  - 예: "CJ ENM, 2100억 공모채 발행…언더금리 확정"
...
(2) **두 번째 문단 첫 문장 — '오늘날짜'와 '최초공시일'을 비교해 아래 두 형식 중 하나만 쓴다. 시제는 data.시제 따른다. 여기서 '무보증 공모 회사채'와 발행일(=청약일)을 함께 명기한다**:
   - ⚠️ **두 형식을 절대 섞지 말 것** — 특히 "…에 따르면"과 "…공시했다"를 한 문장에 함께 쓰지 않는다.
   - **케이스 A — '오늘날짜' ≠ '최초공시일'** (과거에 나온 공시를 지금 정리): **"…에 따르면 … 발행한다/발행했다"** 형식, **'공시했다'는 쓰지 않는다.**
     형식: "[N]일 금융감독원 전자공시시스템에 따르면 [발행사]는 [오는/지난 [발행일]일] [총 규모] 규모의 무보증 공모 회사채를 [시제 어미]"
   ...
   - **케이스 B — '오늘날짜' = '최초공시일'** (오늘 나온 공시를 오늘 기사화, 시제는 대개 '미래'): **'금융감독원…에 따르면'을 붙이지 않고** 문장 끝에 "[N]일 공시했다"만 쓴다.
     형식: "[발행사]는 [오는 [발행일]일] [총 규모] 규모의 무보증 공모 회사채를 발행한다고 [N]일 공시했다."
     예: "롯데쇼핑은 오는 10일 2년물과 3년물로 나눠 총 2600억원 규모의 무보증 공모 회사채를 발행한다고 5일 공시했다."
   - **N은 '오늘날짜'의 일(day)** (예: 2026-06-05 → "5").
   - **발행일은 payload '발행일' 필드의 일(day)** — **회사채는 청약일 = 발행일이므로 본문에 반드시 명기**한다(미래면 "오는 N일", 과거면 "지난 N일" 또는 "N일"). 발행일을 빠뜨리지 말 것.
...
(3) 이후 문단 구성 — 다음 순서를 권장:
   ...
   ④ **최종 발행규모(증액 여부)**: payload **'증액_총_억'이 0보다 크면 증액**. **증액 규모는 반드시 '증액_총_억'(회차 합산) 값**을 쓴다 — 트랜치별 증액액을 합산값처럼 쓰지 말 것.
      형식: "최초 모집액 [회차합산_최초모집_억]억원에서 [증액_총_억]억원 증액해 총 [회차합산_억]억원으로 발행한다/발행했다."
      트랜치별 증액 내역은 뒤에 덧붙여도 된다(예: "2년물·3년물 각각 300억원씩 늘었다"). 단 핵심·헤드라인 수치는 회차 총증액(증액_총_억)이다. **증액_총_억이 0이면 이 단락 생략.**
