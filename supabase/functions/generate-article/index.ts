...
(2) **두 번째 문단 첫 문장 — DCM·IPO·유상증자 모두 반드시 이 도입부로 시작한다(생략 금지). '오늘날짜'와 '최초공시일'을 비교해 아래 두 형식 중 하나만 쓴다. 시제는 data.시제 따른다.**:
   ...
   - ⚠️ **발행일(또는 신주배정기준일·상장일 등 본문 날짜)이 '오늘일'과 같은 날이면**, 도입부에서 이미 "N일"을 썼으므로 뒤에서는 같은 날짜를 반복하지 말고 **"이날"로 받는다.** 예: "5일 금융감독원…에 따르면 경남은행은 **이날** 1000억원 규모의 신종자본증권을 발행했다." (X: "5일 … 경남은행은 6월5일 …")
...
- **'발행한도_총_억'은 회차 전체의 발행 한도(=증액 한도)**다. 트랜치별 한도가 아니므로 "각 만기별 발행 한도는…" 같은 표현 금지.
  - **발행한도가 최초모집보다 크면(증액 여력) 반드시 본문에 명기**한다: "최대 [발행한도_총_억]억원까지 증액 가능하다." 식. **특히 '최종발행_확정=false'(수요예측 전 진행 중 딜)일 때는 '얼마까지 늘 수 있나'가 핵심 정보이므로 적극 활용**한다.
  - 이 경우 **헤드라인 둘째 정보로도 우선** 고려: "…최대 6000억" 처럼. (발행한도가 없거나 최초모집과 같으면 생략.)
